...
### 10.5 M4 终局判定：MMLU 提高到 N=2000 后的配对比较（2026-09-18）

**同一批 2000 道题**分别喂给 BF16 与 4-bit 混合档（同 seed ⇒ 同题目子集，`sample_sha256` 相同）：

| 模型 | MMLU（N=2000） | 标准误 |
|---|---|---|
| BF16 | **0.6260** | ±0.0108 |
| 混合 NVFP4（5 块保护） | **0.5490** | ±0.0111 |
| **差值** | **−7.70 pp** | 合并 SE 1.55 pp → **z = 4.96** |

**判定：确凿显著（p ≪ 0.001），4-bit 档真实损失约 7.7 个百分点。** 决策 16 的 MMLU 门槛（≥0.572）被明确击穿，不再是噪声边界之争。

**抽样波动的现场演示（同一模型、同一配置，仅换 500 道题）**：`0.5560`（N=500 子集 A）vs `0.5080`（N=2000 的前 500 道）——**差 4.8 pp，纯粹来自抽题**。这就是 M1 坚持标注 N 与分辨率的意义，被现场演出来了。

### 10.6 M4 最重要的一条科学结论：困惑度会**低估**任务级损伤
...
**困惑度只涨 1.55%，MMLU 却掉 7.7 个百分点。** 这推翻了本项目早先的假设——M1/M2 曾把困惑度定为「真正的门禁」（因为它稠密、低方差、能分辨亚百分点），把 MMLU 降级为「粗筛」。**在本例中稠密指标严重低估了损伤。**

**修正后的方法论（写进报告）**：
- **稠密指标精确但不充分**：它能分辨 +0.5% 的变化，却看不见「推理能力」这一窄而深的损伤。
- **任务级指标低分辨率但不可替代**：决策 12 保留 MMLU 是有回报的——正是它抓住了困惑度漏掉的东西。
- 结论：**两者必须同时报告，且门禁同时生效**；任何一个单独通过都不构成「无损」。

### 10.7 M4 最终档位表（全部实测）

| 档位 | 困惑度 | vs oracle | MMLU | 体积 | 判定 |
|---|---|---|---|---|---|
| BF16 oracle | 9.3190 | — | 0.6260 (N=2000) | 15.26 GiB | 基准 |
| **FP8 W8A8** | 9.3434 | **+0.26%** | 0.6180 (N=500) | 8.79 GiB | ✅ **两门禁均过 → 部署档** |
| 混合 NVFP4（5 块，修正） | 9.4634 | +1.55% | **0.5490 (N=2000)** | 7.25 GiB | ❌ MMLU −7.70 pp |
| 混合 NVFP4（14 块，bug 版） | 9.3716 | +0.56% | 0.5800 (N=500) | 9.57 GiB | 作废（模式错误） |
| uniform NVFP4 | 9.5799 | +2.80% | 未测 | 待测 | 参考 |
| INT8 SmoothQuant / per-tensor | 14.07 / 19.70 | +51% / +111% | — | — | ❌ 不可用 |

**部署建议**：**选 FP8。** 4-bit 在困惑度上看似可行（+1.55%），但任务级精度损失 7.7 pp 无法接受；在「两个门禁都干净通过」这一标准上，FP8 是唯一达标档位。这也与 M2 的 INT8 失败同源：**该模型/栈上，低于 8 bit 的激活侧承受不起。**
...
## 11. M6 记录：报告与发布（2026-09-18）

**产出**：`README.md`（30 秒读懂的门面：六个档位的表 + 迁移链 + 十条缺陷索引）、`docs/report.md`（完整技术报告：方法 / 结果 / 方法论发现 / 缺陷复盘 / 局限 / 结论）、`LICENSE`（MIT）。

**发布前检查**：`git diff --cached` 无任何 `*.safetensors / *.bin / *.pt / *.engine / *.onnx / *.gguf` → **权重与引擎零入库**；仓库 `.git` 仅 1.6 MB。

**报告的核心叙事（三条）**：
1. **FP8 是部署档**：+0.26% 困惑度、MMLU 在噪声内、体积 −42%、吞吐 1.5–1.7×、TPOT −40%、KV 容量 ×2.28。
2. **8-bit 是地板**：INT8 灾难性失败（+51%/+111%），4-bit 即使做敏感性保护仍损失 7.70 pp MMLU。
3. **稠密指标会低估任务级损伤**：困惑度 +1.55% 而 MMLU −7.70 pp（配对 N=2000, z=4.96）——两个门禁必须同时生效。
...
**双语核对过程中查出两个文档缺陷，同一轮修掉。**（a）`docs/measurement_spec.md` 里 `决策 1` 的理由段落
原先夹在 §1 表格中间；GFM 在遇到第一个非表格行时就结束表格，于是它之后的四行——`max_new_tokens` 2、
`采样` 3、`随机种子` 4、`思考模式` 5——不渲染成表格行，只显示成带竖线的文字。现在该段落移到表格之后。
（b）README 与 report 写「16 条决策」，spec 正文自己写「十五条」，而实际被引用的编号是 14 条
（1–5 与 7–15；6 与 16 未使用，现已在 spec 中写明）。第一次审计这个数字时只匹配了字面量「Decision N」，
漏掉了两张 `| Field | Decision | Value |` 表里那列光秃秃的编号，把总数误报成 11 条——错数在被核对表列之前
已经报给了所有者。


（c）`ROADMAP.en.md` 的代码块里原先留着 32 行中文注释（§0 仓库树、§1.6 结果批注、§10.6 坐标标签）——
那是「代码块逐字保留」规则的副作用：命令是对的，但英文读者必须读的中文被一起冻住了。现只翻译
可读注释，命令、路径与配置键一字未动。校验方式：改动前不含中文的 25 行在改动后逐字仍在原位，
块内数值多重集不变（63 → 63）。
